...
[[nodiscard("Leak")]] token_stream tokenize(string s) {
    ...
    while (true) {
        ...
        // We make direct modifications (just slicing, no allocations) to the _s_ variable
        s = trim_start(s);
        if (!s.Count) break;
        ...
        if (is_digit(s[0])) {
            ...
            auto str = slice(s, 0, end - ch);
            s        = slice(s, end - ch, length(s));

            add(stream.Tokens, token{token::NUMBER, str, fltValue});
        } else if (is_op(s[0])) {
            add(stream.Tokens, token{token::OPERATOR, slice(s, 0, 1)});
            s = slice(s, 1, length(s));
        } else if (is_parenthesis(s[0])) {
            add(stream.Tokens, token{token::PARENTHESIS, slice(s, 0, 1)});
            s = slice(s, 1, length(s));
        } else if (is_alpha(s[0])) {
            add(stream.Tokens, token{token::VARIABLE, slice(s, 0, 1)});
            s = slice(s, 1, length(s));
        } else {
...
token peek(token_stream *stream) {
    if (stream->It.Count) return stream->It[0];
    return token();
}
...
void validate_e(token_stream *stream) {
    ...
    while (!stream->Error.Count && (peek(stream).Type == token::OPERATOR || is_v(peek(stream)) || strings_match(peek(stream).Str, "("))) {
...
void pop(any_dynamic_array_like auto ref arr) {
    remove_unordered_at_index(arr, -1);
}

void pop_op(array<token> ref ops, array<ast *> ref operands) {
    if (ops[-1].Unary) {
        auto *t0 = operands[-1];
        pop(operands);
...
void push_op(token op, array<token> ref ops, array<ast *> ref operands) {
    while (ops[-1] > op) {
        pop_op(ops, operands);
    }
    add(ops, op);
...
void parse_e(token_stream *stream, array<token> ref ops, array<ast *> ref operands);

void parse_p(token_stream *stream, array<token> ref ops, array<ast *> ref operands) {
    auto next = peek(stream);
    if (is_v(next)) {
