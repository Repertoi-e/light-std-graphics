...
inline bool operator==(const token &a, const token &b) { return a.Type == b.Type && strings_match(a.Str, b.Str); }
...
inline void error(token_stream *stream, string message, s64 errorPos = -1) {
    if (stream->Error.Count) return;
    stream->Error += message;
    stream->Error += "\n  ";
    stream->Error += stream->Expression;

    if (errorPos == -1) {
        if (stream->It.Count) {
            errorPos = stream->It[0].Str.Data - stream->Expression.Data;
        } else {
            errorPos = length(stream->Expression);
        }
    }
    stream->Error += mprint("\n  {: >{}}", "^", errorPos + 1);
}
