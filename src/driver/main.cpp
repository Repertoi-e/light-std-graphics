...
s32 main() {
    ...
    auto *data = (tlsf_allocator_data *) pack;
    pack += sizeof(tlsf_allocator_data);
    auto *m = (memory *) pack;
	pack += sizeof(memory);
	auto* g = (graphics*)pack;
	pack += sizeof(graphics);
	auto* pool = pack;
	...
	m->MainWindow.GetPos = []() { return get_pos(MainWindow); };
	m->MainWindow.GetCursorPos = []() { return get_cursor_pos(MainWindow); };
	...
	m->MainWindow.GetFlags = []() { return get_flags(MainWindow); };
    ...
    m->GetStateImpl = [](string name, s64 size, bool *created) {
        ...
        auto **found = search(Memory->States, identifier).Value;
        if (!found) {
...
file_scope void init_imgui_for_our_windows() {
    ...
    io.GetClipboardTextFn = [](auto) {
        ...
        memcpy(buffer.Data, content.Data, content.Count);
        buffer.Count          = content.Count + 1;
        buffer.Data[content.Count] = 0;
    ...
    bd->MouseCursors[0] = make_cursor(CURSOR_ARROW);
    bd->MouseCursors[1] = make_cursor(CURSOR_IBEAM);
    bd->MouseCursors[2] = make_cursor(CURSOR_RESIZE_ALL);
    bd->MouseCursors[3] = make_cursor(CURSOR_RESIZE_NS);
    bd->MouseCursors[4] = make_cursor(CURSOR_RESIZE_WE);
    bd->MouseCursors[5] = make_cursor(CURSOR_RESIZE_NESW);
    bd->MouseCursors[6] = make_cursor(CURSOR_RESIZE_NWSE);
    bd->MouseCursors[7] = make_cursor(CURSOR_HAND);
}
...
file_scope void imgui_for_our_windows_new_frame() {
    ...
    memset0(bd->MouseJustPressed, sizeof(bd->MouseJustPressed));
