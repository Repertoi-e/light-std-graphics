...
export BOOL platform_is_windows_10_build_or_greater(WORD build) {
...
// @ThreadSafety
signal<void(monitor_event)> MonitorEvent;

monitor* create_monitor(DISPLAY_DEVICEW* adapter, DISPLAY_DEVICEW* display) {
	DEVMODEW dm;
	memset0(&dm, sizeof(dm));
	dm.dmSize = sizeof(dm);
	...
	memcpy(mon->PlatformData.Win32.AdapterName, adapter->DeviceName, c_string_length(adapter->DeviceName) * sizeof(wchar_t));
	...
	if (display) {
		memcpy(mon->PlatformData.Win32.DisplayName, display->DeviceName, c_string_length(adapter->DeviceName) * sizeof(wchar_t));
...
void do_monitor_event(monitor* mon, monitor_event::action action, bool insertLast) {
...
void split_bpp(s32 bpp, s32* red, s32* green, s32* blue) {
	s32 delta;
	if (bpp == 32) bpp = 24;
...
display_mode choose_video_mode(monitor* mon, display_mode desired) {
	u32 sizeDiff, leastSizeDiff = numeric<u32>::max();
	u32 rateDiff, leastRateDiff = numeric<u32>::max();
	u32 colorDiff, leastColorDiff = numeric<u32>::max();
	...
	For(mon->DisplayModes) {
		...
		if (desired.RefreshRate != display_mode::DONT_CARE) {
			...
			rateDiff = numeric<u32>::max() - it.RefreshRate;
		}
...
export {
	void platform_poll_monitors();

	// This needs to be handled by the runtime
	// :AfterMain:
	void platform_monitor_uninit() {
		free(MonitorEvent);
	...
	void platform_monitor_init() {
		...
		PUSH_ALLOC(platform_get_persistent_allocator()) {
			reserve(Monitors, 8);
			reserve(MonitorEvent.Callbacks, 8);
		}
	...
	void platform_poll_monitors() {
		...
		for (DWORD adapterIndex = 0;; adapterIndex++) {
			...
			memset0(&adapter, sizeof(adapter));
			...
			while (true) {
				memset0(&display, sizeof(display));
...
export {
	...
	void poll_monitors() { platform_poll_monitors(); }
