...
export {
    ...
    template <typename R, typename... Args>
    void emit_while_false(signal<R(Args...)> * s, Args ref... args) {
    ...
    template <typename R, typename... Args>
    void emit_while_true(signal<R(Args...)> * s, Args ref... args) {
        static_assert(types::is_convertible<R, bool>);
    ...
    void free_signal(signal<T> * s) {
        // @Cleanup @Performance Make it a stack array
        if (s->Callbacks.Data) free(s->Callbacks.Data);
        if (s->ToRemove.Data) free(s->ToRemove.Data);
    }
}
