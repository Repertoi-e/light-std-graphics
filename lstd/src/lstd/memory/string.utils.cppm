...
export module lstd.string.utils;
...
export {
    ...
    constexpr code_point decode_cp(const char *str) {
        if (0xf0 == (0xf8 & str[0])) {
    ...
    constexpr const char *utf8_get_cp_at_index(const char *str, s64 index) {
        For(range(index)) str += utf8_get_size_of_cp(str);
        return str;
    }
    ...
    // Also adds a null-terminator at the end.
    constexpr void utf8_to_utf32(const char *str, s64 length, code_point *out) {
        For(range(length)) {
            code_point cp = decode_cp(str);
            *out++        = cp;
            str += utf8_get_size_of_cp(cp);
    ...
    constexpr void utf16_to_utf8(const wchar *str, char *out, s64 *outByteLength) {
        ...
        while (*str) {
            ...
            if ((cp >= 0xD800) && (cp <= 0xDBFF)) {
                ...
                if (!*str) assert(false && "Invalid wchar string");

                if ((trail >= 0xDC00) && (trail <= 0xDFFF)) {
                    cp = ((cp - 0xD800) << 10) + (trail - 0xDC00) + 0x0010000;
                } else {
                    assert(false && "Invalid wchar string");
                }
            }
