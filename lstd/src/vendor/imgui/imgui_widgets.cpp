...
bool ImGui::ButtonBehavior(const ImRect &bb, ImGuiID id, bool *out_hovered, bool *out_held, ImGuiButtonFlags flags) {
    ...
    if (id != 0 && window->DC.LastItemId != id) ImGuiTestEngineHook_ItemAdd(&g, bb, id);
#endif
    ...
    if (g.NavId == id && !g.NavDisableHighlight && g.NavDisableMouseHover &&
        ...
        if (!(flags & ImGuiButtonFlags_NoHoveredOnNav)) hovered = true;
            ...
            if (g.IO.MouseDown[0]) {
                ...
                if (hovered && (flags & ImGuiButtonFlags_PressedOnClickRelease) && !g.DragDropActive) {
                    bool is_double_click_release =
                        (flags & ImGuiButtonFlags_PressedOnDoubleClick) && g.IO.MouseDownWasDoubleClick[0];
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2 &pos)  //, float size)
{
    ...
    if (hovered) window->DrawList->AddCircleFilled(center, ImMax(2.0f, g.FontSize * 0.5f + 1.0f), col, 12);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2 &pos, ImGuiDockNode *dock_node) {
    ...
    ImVec2 off =
        dock_node ? ImVec2((float) (int) (-g.Style.ItemInnerSpacing.x * 0.5f) + 0.5f, 0.0f) : ImVec2(0.0f, 0.0f);
    ImU32 bg_col =
        GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    ImU32 text_col = GetColorU32(ImGuiCol_Text);
    ImVec2 center = bb.GetCenter();
    ...
    if (IsItemActive() && IsMouseDragging(0)) StartMouseDragFromTitleBar(window, dock_node, true);
...
bool ImGui::ScrollbarEx(const ImRect &bb_frame, ImGuiID id, ImGuiAxis axis, float *p_scroll_v, float size_avail_v,
    ...
    // facilitate using the resize grab)
    float alpha = 1.0f;
...
static int IMGUI_CDECL ShrinkWidthItemComparer(const ImGuiShrinkWidthItem *a, const ImGuiShrinkWidthItem *b) {
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem *items, int count, float width_excess) {
    ...
    ImQsort(items, items + count, ShrinkWidthItemComparer);
...
int ImGui::DataTypeFormatString(char *buf, int buf_size, ImGuiDataType data_type, const void *data_ptr,
                                const char *format) {
    ...
    if (data_type == ImGuiDataType_S8) return ImFormatString(buf, buf_size, format, *(const ImS8 *) data_ptr);
    if (data_type == ImGuiDataType_U8) return ImFormatString(buf, buf_size, format, *(const ImU8 *) data_ptr);
    if (data_type == ImGuiDataType_S16) return ImFormatString(buf, buf_size, format, *(const ImS16 *) data_ptr);
    if (data_type == ImGuiDataType_U16) return ImFormatString(buf, buf_size, format, *(const ImU16 *) data_ptr);
    IM_ASSERT(0);
    return 0;
}
...
bool ImGui::DataTypeApplyOpFromText(const char *buf, const char *initial_value_buf, ImGuiDataType data_type,
    ...
    if (data_type == ImGuiDataType_S32) {
        ...
        // We don't bother handling support for legacy operators since they are a little too crappy. Instead we will
        // later implement a proper expression evaluator in the future.
...
bool ImGui::DragScalar(const char *label, ImGuiDataType data_type, void *v, float v_speed, const void *v_min,
    ...
    if (temp_input_is_active || temp_input_start) return TempInputTextScalar(frame_bb, id, label, data_type, v, format);
...
bool ImGui::SliderBehavior(const ImRect &bb, ImGuiID id, ImGuiDataType data_type, void *v, const void *v_min,
    ...
    switch (data_type) {
        case ImGuiDataType_S8: {
            ImS32 v32 = (ImS32) * (ImS8 *) v;
            bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS8 *) v_min,
                                                          *(const ImS8 *) v_max, format, power, flags, out_grab_bb);
            if (r) *(ImS8 *) v = (ImS8) v32;
            return r;
        }
        case ImGuiDataType_U8: {
            ImU32 v32 = (ImU32) * (ImU8 *) v;
            bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU8 *) v_min,
                                                          *(const ImU8 *) v_max, format, power, flags, out_grab_bb);
            if (r) *(ImU8 *) v = (ImU8) v32;
            return r;
        }
        case ImGuiDataType_S16: {
            ImS32 v32 = (ImS32) * (ImS16 *) v;
            bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS16 *) v_min,
                                                          *(const ImS16 *) v_max, format, power, flags, out_grab_bb);
            if (r) *(ImS16 *) v = (ImS16) v32;
            return r;
        }
        case ImGuiDataType_U16: {
            ImU32 v32 = (ImU32) * (ImU16 *) v;
            bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU16 *) v_min,
                                                          *(const ImU16 *) v_max, format, power, flags, out_grab_bb);
            if (r) *(ImU16 *) v = (ImU16) v32;
            return r;
        }
        case ImGuiDataType_S32:
...
bool ImGui::TempInputTextScalar(const ImRect &bb, ImGuiID id, const char *label, ImGuiDataType data_type,
                                void *data_ptr, const char *format) {
    ImGuiContext &g = *GImGui;
    ...
    if (value_changed) {
        value_changed =
            DataTypeApplyOpFromText(data_buf, g.InputTextState.InitialTextA.Data, data_type, data_ptr, NULL);
        if (value_changed) MarkItemEdited(id);
    }
    return value_changed;
}
...
bool ImGui::InputScalar(const char *label, ImGuiDataType data_type, void *data_ptr, const void *step,
    ...
    flags |= ImGuiInputTextFlags_NoMarkEdited;  // We call MarkItemEdited() ourselve by comparing the actual data rather
                                                // than the string.
    ...
    if (step != NULL) {
        ...
        if (InputText("", buf, IM_ARRAYSIZE(buf),
            ...
            value_changed =
                DataTypeApplyOpFromText(buf, g.InputTextState.InitialTextA.Data, data_type, data_ptr, format);
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf, int buf_size, const ImVec2 &size_arg,
    ...
    const bool focus_requested_by_code =
        focus_requested &&
        (g.FocusRequestCurrWindow == window && g.FocusRequestCurrCounterAll == window->DC.FocusCounterAll);
    ...
    if (init_state && g.ActiveId != id) {
        ...
        // FIXME: For non-readonly widgets we might be able to require that TextAIsValid && TextA == buf ? (untested)
        // and discard undo stack if user buffer has changed.
    ...
    if (render_cursor || render_selection) {
        ...
        if (is_multiline)
            text_size = ImVec2(size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_display_end) *
...
bool ImGui::ColorEdit4(const char *label, float col[4], ImGuiColorEditFlags flags) {
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 &&
        ...
        const float w_item_one = ImMax(
            1.0f, (float) (int) ((w_inputs - (style.ItemInnerSpacing.x) * (components - 1)) / (float) components));
        const float w_item_last =
            ImMax(1.0f, (float) (int) (w_inputs - (w_item_one + style.ItemInnerSpacing.x) * (components - 1)));
        ...
        for (int n = 0; n < components; n++) {
            ...
            if (flags & ImGuiColorEditFlags_Float) {
                value_changed |=
                    DragFloat(ids[n], &f[n], 1.0f / 255.0f, disable_hue_edit ? +FLT_MAX : 0.0f,
                              disable_hue_edit ? -FLT_MAX : hdr ? 0.0f : 1.0f, fmt_table_float[fmt_idx][n]);
                value_changed_as_float |= value_changed;
            } else {
                value_changed |= DragInt(ids[n], &i[n], 1.0f, disable_hue_edit ? INT_MAX : 0,
                                         disable_hue_edit ? INT_MIN : hdr ? 0 : 255, fmt_table_int[fmt_idx][n]);
            }
...
bool ImGui::ColorPicker4(const char *label, float col[4], ImGuiColorEditFlags flags, const float *ref_col) {
    ...
    if (alpha_bar) {
        ...
        RenderColorRectWithAlphaCheckerboard(bar1_bb.Min, bar1_bb.Max, 0, bar1_bb.GetWidth() / 2.0f,
                                             ImVec2(0.0f, 0.0f));
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char *label, const char *label_end) {
    ...
    ImRect frame_bb =
        ImRect(window->DC.CursorPos, ImVec2(window->WorkRect.Max.x, window->DC.CursorPos.y + frame_height));
    if (display_frame) {
    ...
    if (display_frame) {
        ...
        RenderArrow(window->DrawList, frame_bb.Min + ImVec2(padding.x, text_base_offset_y), text_col,
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            RenderBullet(window->DrawList,
                         frame_bb.Min + ImVec2(text_offset_x * 0.5f, g.FontSize * 0.50f + text_base_offset_y),
                         text_col);
        else if (!is_leaf)
            RenderArrow(window->DrawList, frame_bb.Min + ImVec2(padding.x, g.FontSize * 0.15f + text_base_offset_y),
...
bool ImGui::CollapsingHeader(const char *label, bool *p_open, ImGuiTreeNodeFlags flags) {
    ...
    flags |= ImGuiTreeNodeFlags_CollapsingHeader |
             (p_open ? ImGuiTreeNodeFlags_AllowItemOverlap | ImGuiTreeNodeFlags_ClipLabelForTrailingButton : 0);
...
float ImGuiMenuColumns::CalcExtraSpace(float avail_w) { return ImMax(0.0f, avail_w - Width); }
...
bool ImGui::BeginMainMenuBar() {
    ...
    SetNextWindowSize(
        ImVec2(viewport->Size.x, g.NextWindowData.MenuBarOffsetMinVal.y + g.FontBaseSize + g.Style.FramePadding.y));
    SetNextWindowViewport(viewport->ID);  // Enforce viewport so we don't create our onw viewport when
...
bool ImGui::BeginMenuBar() {
    ...
    ImRect clip_rect(
        ImFloor(bar_rect.Min.x + window->WindowBorderSize + 0.5f),
        ImFloor(bar_rect.Min.y + window->WindowBorderSize + 0.5f),
        ImFloor(ImMax(bar_rect.Min.x, bar_rect.Max.x - ImMax(window->WindowRounding, window->WindowBorderSize)) + 0.5f),
...
bool ImGui::BeginMenu(const char *label, bool enabled) {
    ...
    // menu, However the final position is going to be different! It is choosen by FindBestWindowPosForPopup(). e.g.
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        RenderArrow(window->DrawList, pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.30f, 0.0f),
        ...
        ImGuiLayoutType_Vertical)  // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && child_menu_window != NULL && !(window->Flags & ImGuiWindowFlags_MenuBar)) {
            ...
            ImVec2 tb =
                (window->Pos.x < child_menu_window->Pos.x) ? next_window_rect.GetTL() : next_window_rect.GetTR();
            ImVec2 tc =
                (window->Pos.x < child_menu_window->Pos.x) ? next_window_rect.GetBL() : next_window_rect.GetBR();
            float extra = ImClamp(ImFabs(ta.x - tb.x) * 0.30f, 5.0f, 30.0f);     // add a bit of extra slack.
            ta.x += (window->Pos.x < child_menu_window->Pos.x) ? -0.5f : +0.5f;  // to avoid numerical issues
            tb.y = ta.y + ImMax((tb.y - extra) - ta.y,
                                -100.0f);  // triangle is maximum 200 high to limit the slope and the bias toward large
                                           // sub-menus // FIXME: Multiply by fb_scale?
...
bool ImGui::MenuItem(const char *label, const char *shortcut, bool selected, bool enabled) {
    ...
    ImGuiSelectableFlags flags = ImGuiSelectableFlags_PressedOnRelease | ImGuiSelectableFlags_SetNavIdOnHover |
...
static int IMGUI_CDECL TabItemComparerByVisibleOffset(const ImGuiTabItem *a, const ImGuiTabItem *b) {
...
bool ImGui::BeginTabBarEx(ImGuiTabBar *tab_bar, const ImRect &tab_bar_bb, ImGuiTabBarFlags flags,
    ...
    if ((flags & ImGuiTabBarFlags_Reorderable) && !(tab_bar->Flags & ImGuiTabBarFlags_Reorderable) &&
        ...
        auto *data = tab_bar->Tabs.Data;
        ImQsort(data, data + tab_bar->Tabs.Size, TabItemComparerByVisibleOffset);
    }
...
void ImGui::TabBarAddTab(ImGuiTabBar *tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow *window) {
    ...
    IM_ASSERT(g.CurrentTabBar ==
              NULL);  // Can't work while the tab bar is active as our tab doesn't have an X offset yet
...
static ImGuiTabItem *ImGui::TabBarScrollingButtons(ImGuiTabBar *tab_bar) {
    ...
    if (select_dir != 0)
        if (ImGuiTabItem *tab_item = TabBarFindTabByID(tab_bar, tab_bar->SelectedTabId)) {
            ...
            tab_to_select = &tab_bar->Tabs[(target_order >= 0 && target_order < tab_bar->Tabs.Size)
                                               ? target_order
                                               : selected_order];  // If we are at the end of the list, still scroll to
                                                                   // make our tab visible
        }
...
bool ImGui::TabItemEx(ImGuiTabBar *tab_bar, const char *label, bool *p_open, ImGuiTabItemFlags flags,
    ...
    if (p_open == NULL) flags |= ImGuiTabItemFlags_NoCloseButton;
