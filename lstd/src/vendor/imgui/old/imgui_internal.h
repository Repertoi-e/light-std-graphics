...
#define ImQsort LSTD_NAMESPACE::quick_sort
...
// long float/double (Exceptionally using templates here but we could also redefine them for variety of types)
template <typename T>
...
// Helper: ImPool<>. Basic keyed storage for contiguous instances, slow/amortized insertion, O(1) indexable, O(Log N)
// queries by ID over a dense/hot buffer, Honor constructor/destructor. Add/remove invalidate all pointers. Indexes have
// the same lifetime as the associated object.
...
enum ImGuiItemStatusFlags_ {
    ...
    ImGuiItemStatusFlags_HasDeactivated =
        1 << 4,  // Set if the widget/group is able to provide data for the ImGuiItemStatusFlags_Deactivated flag.
...
// 1D vector (this odd construct is used to facilitate the transition between 1D and 2D, and the maintenance of some
// branches/patches)
struct ImVec1 {
    float x;
    ImVec1() { x = 0.0f; }
...
struct IMGUI_API ImGuiInputTextState {
    ...
    int CurLenW, CurLenA;  // we need to maintain our buffer length in both UTF-8 and wchar format. UTF-8 len is valid
...
enum ImGuiDockNodeFlagsPrivate_ {
    ...
    ImGuiDockNodeFlags_LocalFlagsMask_ =
        ImGuiDockNodeFlags_NoSplit | ImGuiDockNodeFlags_NoResize | ImGuiDockNodeFlags_AutoHideTabBar |
        ImGuiDockNodeFlags_DockSpace | ImGuiDockNodeFlags_CentralNode | ImGuiDockNodeFlags_NoTabBar |
...
struct ImGuiContext {
    ...
    ImGuiID NavJustMovedToMultiSelectScopeId;  // Just navigated to this select scope id (result of a successfully
                                               // MoveRequest).
    ...
    ImGuiWindow *NavWindowingTarget;      // When selecting a window (holding Menu+FocusPrev/Next, or equivalent of
                                          // CTRL-TAB) this window is temporarily displayed top-most.
    ...
    int FocusRequestCurrCounterAll;       // Any item being requested for focus, stored as an index (we on layout to be
...
struct IMGUI_API ImGuiWindowTempData {
    ...
    ImVec2 CursorMaxPos;    // Used to implicitly calculate the size of our contents, always growing during the frame.
                            // Used to calculate window->ContentSize at the beginning of next frame
    ImVec2 CurrLineSize;
    ImVec2 PrevLineSize;
    float CurrLineTextBaseOffset;
    float PrevLineTextBaseOffset;
    int TreeDepth;
    ImU32 TreeStoreMayJumpToParentOnPop;  // Store a copy of !g.NavIdIsAlive for TreeDepth 0..31.. Could be turned into
    ...
    int FocusCounterAll;               // Counter for focus/tabbing system. Start at -1 and increase as assigned via
...
struct IMGUI_API ImGuiWindow {
    ...
    ImRect WorkRect;  // Cover the whole scrolling region, shrunk by WindowPadding*1.0f on each side. This is meant to
                      // replace ContentsRegionRect over time (from 1.71+ onward).
    ...
    ImRect ContentsRegionRect;  // FIXME: This is currently confusing/misleading. It is essentially WorkRect but not
...
enum ImGuiTabItemFlagsPrivate_ {
    ...
    ImGuiTabItemFlags_Unsorted = 1 << 21,  // [Docking] Trailing tabs with the _Unsorted flag will be sorted based on
...
IMGUI_API bool IsPopupOpen(ImGuiID id);  // Test for id within current popup stack level (currently begin-ed into); this
...
// guarantee stability.
// - You can create dockspace _or_ floating nodes with this API. To create a dockspace node, make sure to set the
// ImGuiDockNodeFlags_DockSpace flag.
// - If you intend to split the node immediately after creation using DockBuilderSplitNode(), make sure to call
// DockBuilderSetNodeSize() beforehand.
// - Call DockBuilderFinish() after you are done.
// - Important: do not hold on ImGuiDockNode* pointers! They may be invalidated by any split/merge/remove operation and
// every frame.
...
IMGUI_API void DockBuilderRemoveNodeChildNodes(
    ImGuiID node_id);  // Remove all split/hierarchy. All remaining docked windows will be re-docked to the root.
...
IMGUI_API int DataTypeFormatString(char *buf, int buf_size, ImGuiDataType data_type, const void *data_ptr,
                                   const char *format);
...
IMGUI_API bool TempInputTextScalar(const ImRect &bb, ImGuiID id, const char *label, ImGuiDataType data_type,
                                   void *data_ptr, const char *format);
