...
/*
 ...
 are using:
         ...
         OpenGL:     ImTextureID = GLuint                       (see ImGui_ImplOpenGL3_RenderDrawData() function in
 imgui_impl_opengl3.cpp) DirectX9:   ImTextureID = LPDIRECT3DTEXTURE9           (see ImGui_ImplDX9_RenderDrawData()
...
#define toupper LSTD_NAMESPACE::to_upper
...
static const float WINDOWS_MOUSE_WHEEL_SCROLL_LOCK_TIMER =
    2.00f;  // Lock scrolled window (so it doesn't pick child windows that are scrolling through) for a certaint time,
            // unless mouse moved.
...
void ImStrncpy(char *dst, const char *src, size_t count) {
    if (count < 1) return;
    if (count > 1) my_strncpy(dst, src, count - 1);
    dst[count - 1] = 0;
}
...
const char *ImStrchrRange(const char *str, const char *str_end, char c) {
    auto s = LSTD_NAMESPACE::string(str, str_end - str);
    auto r = LSTD_NAMESPACE::find_cp(s, c);
    if (r == -1) return null;
    return str + r;
}
...
const char *ImStreolRange(const char *str, const char *str_end) {
    auto s = LSTD_NAMESPACE::string(str, str_end - str);
    auto r = LSTD_NAMESPACE::find_cp(s, '\n');
    if (r == -1) return str_end;
    return str + r;
}
...
void ImGuiStorage::BuildSortByKey() {
    ...
    if (Data.Size > 1) ImQsort(Data.Data, Data.Data + Data.Size, StaticFunc::PairCompareByID);
}
...
void ImGui::StartMouseDragFromTitleBar(ImGuiWindow *window, ImGuiDockNode *node, bool from_collapse_button) {
    ...
    if (node != NULL && node->VisibleWindow && (node->VisibleWindow->Flags & ImGuiWindowFlags_NoMove) == 0) {
        ...
        if (root_node->OnlyNodeWithWindows != node || (root_node->CentralNode != NULL))
            if (from_collapse_button || root_node->IsDockSpace()) can_extract_dock_node = true;
    }
        ...
        if (g.HoveredRootWindow != NULL) {
            ...
            if (g.IO.ConfigWindowsMoveFromTitleBarOnly &&
                (!(g.HoveredRootWindow->Flags & ImGuiWindowFlags_NoTitleBar) ||
                 g.HoveredWindow->RootWindowDockStop->DockIsActive))
...
void ImGui::NewFrame() {
    ...
    if (g.IO.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) {
        ...
        for (int monitor_n = 0; monitor_n < g.PlatformIO.Monitors.Size; monitor_n++) {
            ...
            IM_ASSERT(mon.WorkSize.x > 0.0f && mon.WorkSize.y > 0.0f &&
                      "Monitor bounds not setup properly. If you don't have work area information, just copy Min/Max "
                      "into them.");
    ...
    for (int n = 0; n < g.Viewports.Size; n++) virtual_space.Add(g.Viewports[n]->GetRect());
    ...
    const float memory_compact_start_time = (g.IO.ConfigWindowsMemoryCompactTimer >= 0.0f)
                                                ? (float) g.Time - g.IO.ConfigWindowsMemoryCompactTimer
                                                : FLT_MAX;
...
static int IMGUI_CDECL ChildWindowComparer(ImGuiWindow *const *lhs, ImGuiWindow *const *rhs) {
...
static void AddWindowToSortBuffer(ImVector<ImGuiWindow *> *out_sorted_windows, ImGuiWindow *window) {
    ...
    if (window->Active) {
        ...
        if (count > 1) {
            auto *data = window->DC.ChildWindows.Data;
            ImQsort(data, data + count, ChildWindowComparer);
        }
...
static void ImGui::EndFrameDrawDimmedBackgrounds() {
    ...
    if (dim_bg_for_modal || dim_bg_for_window_list)
        for (int viewport_n = 0; viewport_n < g.Viewports.Size; viewport_n++) {
            ...
            if (g.NavWindowingList && viewport == g.NavWindowingList->Viewport) continue;
    ...
    if (dim_bg_for_window_list) {
        ...
        if (bb.Contains(
                window->Viewport->GetRect()))  // If a window fits the entire viewport, adjust its highlight inward
        {
    ...
    for (int n = 0; n < IM_ARRAYSIZE(windows_to_render_top_most); n++)
        if (windows_to_render_top_most[n] &&
            IsWindowActiveAndVisible(windows_to_render_top_most[n]))  // NavWindowingTarget is always temporarily
                                                                      // displayed as the tp-most window
            AddRootWindowToDrawData(windows_to_render_top_most[n]);

    // Draw software mouse cursor if requested
    if (g.IO.MouseDrawCursor) RenderMouseCursor(g.IO.MousePos, g.Style.MouseCursorScale, g.MouseCursor);
...
bool ImGui::IsMouseHoveringRect(const ImVec2 &r_min, const ImVec2 &r_max, bool clip) {
    ...
    if (!g.MouseViewport->GetRect().Overlaps(rect_clipped)) return false;
    return true;
}
...
static ImGuiWindow *CreateNewWindow(const char *name, ImVec2 size, ImGuiWindowFlags flags) {
    ...
    if (!(flags & ImGuiWindowFlags_NoSavedSettings))
        if (ImGuiWindowSettings *settings = ImGui::FindWindowSettings(window->ID)) {
            ...
            if (settings->Size.x > 0 && settings->Size.y > 0) size = ImVec2(settings->Size.x, settings->Size.y);
...
static bool ImGui::UpdateManualResize(ImGuiWindow *window, const ImVec2 &size_auto_fit, int *border_held,
    ...
    // Clip mouse interaction rectangles within the viewport (in practice the narrowing is going to happen most of the
    // time).
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++) {
        ...
        if (resize_grip_n == 0 || held || hovered)
            resize_grip_col[resize_grip_n] = GetColorU32(
                held ? ImGuiCol_ResizeGripActive : hovered ? ImGuiCol_ResizeGripHovered
                                                           : ImGuiCol_ResizeGrip);
    }
    ...
    if (window->Collapsed) {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground)) {
            ...
            if (window->ViewportOwned) {
                ...
                if (is_docking_transparent_payload) alpha *= DOCKING_TRANSPARENT_PAYLOAD_ALPHA;
                if (alpha != 1.0f)
        ...
        if (window->DockNode && window->DockNode->IsHiddenTabBar() && !window->DockNode->IsNoTabBar()) {
            ...
            ImU32 col = GetColorU32(((held && hovered) || (window->DockNode->IsFocused && !hovered))
...
bool ImGui::Begin(const char *name, bool *p_open, ImGuiWindowFlags flags) {
    ...
    if (first_begin_of_the_frame) {
        ...
        else if (g.NavWindowingList != NULL &&
        ...
        if (window->ViewportOwned) {
            ...
            // Either a SetNextWindowPos() call in the current frame or a SetWindowPos() call in the previous frame may
            // have this effect.
            if (viewport_rect_changed) UpdateViewportPlatformMonitor(window->Viewport);

            // Update common viewport flags
            ImGuiViewportFlags viewport_flags =
                (window->Viewport->Flags) &
            ...
            // For popups and menus that may be protruding out of their parent viewport, we enable _NoFocusOnClick so
            // that clicking on them won't steal the OS focus away from their parent window (which may be reflected in
            // OS the title bar decoration). Setting _NoFocusOnClick would technically prevent us from bringing back to
            // front in case they are being covered by an OS window from a different app, but it shouldn't be much of a
            // problem considering those are already popups that are closed when clicking elsewhere.
            ...
            if (window->WindowClass.ParentViewportId)
                ...
                window->Viewport->ParentViewportId =
                    g.IO.ConfigViewportsNoDefaultParent ? 0 : IMGUI_VIEWPORT_DEFAULT_ID;
            if (window->WindowClass.ViewportFlagsOverrideSet)
                viewport_flags |= window->WindowClass.ViewportFlagsOverrideSet;
            if (window->WindowClass.ViewportFlagsOverrideClear)
                viewport_flags &= ~window->WindowClass.ViewportFlagsOverrideClear;

            // We also tell the back-end that clearing the platform window won't be necessary, as our window is filling
            // the viewport and we have disabled BgAlpha
        ...
        if (!window_pos_set_by_api && !(flags & ImGuiWindowFlags_ChildWindow) && window->AutoFitFramesX <= 0 &&
            ...
            if (!window->ViewportOwned && viewport_rect.GetWidth() > 0 && viewport_rect.GetHeight() > 0.0f)
                ...
                if (window->Viewport->PlatformMonitor == -1) {
                    // Fallback for "lost" window (e.g. a monitor disconnected): we move the window back over the main
                    // viewport
        ...
        if (window->ViewportOwned) window->WindowRounding = 0.0f;
        ...
        const bool dim_bg_for_window_list =
            g.NavWindowingTargetAnim && ((window == g.NavWindowingTargetAnim->RootWindow) ||
                                         (g.NavWindowingList && (window == g.NavWindowingList) &&
                                          g.NavWindowingList->Viewport != g.NavWindowingTargetAnim->Viewport));
        if (dim_bg_for_modal || dim_bg_for_window_list) {
            const ImU32 dim_bg_col =
                GetColorU32(dim_bg_for_modal ? ImGuiCol_ModalWindowDimBg : ImGuiCol_NavWindowingDimBg, g.DimBgRatio);
        ...
        */
        ...
        if (g.IO.ConfigFlags & ImGuiConfigFlags_DockingEnable) {
            ...
            // BeginAsDockableDragDropSource() also overwrites it.
            ...
            // BeginAsDockableDragDropTarget() in DockNodeUpdate() instead.
    ...
    // being hidden from one frame. It is especially important as e.g. nested TabBars would otherwise generate flicker
    // in the form of one empty frame, or focus requests won't be processed.
    if (window->DockIsActive && !window->DockTabIsVisible) {
    ...
    for (int n = (*last_dir != ImGuiDir_None) ? -1 : 0; n < ImGuiDir_COUNT; n++) {
        ...
        pos.x = (dir == ImGuiDir_Left)    ? r_avoid.Min.x - size.x
                : (dir == ImGuiDir_Right) ? r_avoid.Max.x
                                          : base_pos_clamped.x;
        pos.y =
            (dir == ImGuiDir_Up) ? r_avoid.Min.y - size.y : (dir == ImGuiDir_Down) ? r_avoid.Max.y
                                                                                   : base_pos_clamped.y;
        *last_dir = dir;
        return pos;
    }
...
static void ImGui::NavUpdateWindowing() {
    ...
    if (apply_toggle_layer && g.NavWindow) {
        ...
        // When entering a regular menu bar with the Alt key, we always reinitialize the navigation ID. It however
        // persist on docking tab tabs.
...
static void SettingsHandlerWindow_WriteAll(ImGuiContext *ctx, ImGuiSettingsHandler *handler, ImGuiTextBuffer *buf) {
    ...
    for (int i = 0; i != g.SettingsWindows.Size; i++) {
        ...
        if (settings->DockId != 0) {
            // Write DockId as 4 digits if possible. Automatic DockId are small numbers, but full explicit DockSpace()
            // are full ImGuiID range.
...
static bool ImGui::UpdateTryMergeWindowIntoHostViewport(ImGuiWindow *window, ImGuiViewportP *viewport) {
    ImGuiContext &g = *GImGui;
    if (!(viewport->Flags & (ImGuiViewportFlags_CanHostOtherWindows | ImGuiViewportFlags_Minimized)) ||
        window->Viewport == viewport)
        return false;
    if (!viewport->GetRect().Contains(window->Rect())) return false;
    ...
    for (int n = 0; n < g.Windows.Size; n++) {
        ...
        if (window_behind->WasActive && window_behind->ViewportOwned &&
            ...
            if (window_behind->Viewport->GetRect().Overlaps(window->Rect())) return false;
    }
...
static ImGuiViewportP *FindHoveredViewportFromPlatformWindowStack(const ImVec2 mouse_platform_pos) {
    ...
    for (int n = 0; n < g.Viewports.Size; n++) {
        ...
        if (!(viewport->Flags & (ImGuiViewportFlags_NoInputs | ImGuiViewportFlags_Minimized)) &&
            viewport->GetRect().Contains(mouse_platform_pos))
...
static void ImGui::UpdateViewportsNewFrame() {
    ...
    if (g.ConfigFlagsCurrFrame & ImGuiConfigFlags_ViewportsEnable)
        main_viewport_platform_pos = (main_viewport->Flags & ImGuiViewportFlags_Minimized)
                                         ? main_viewport->Pos
                                         : g.PlatformIO.Platform_GetWindowPos(main_viewport);
    AddUpdateViewport(NULL, IMGUI_VIEWPORT_DEFAULT_ID, main_viewport_platform_pos, main_viewport_platform_size,
    ...
    if (g.IO.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport) {
        ...
        // If the back-end doesn't know how to honor ImGuiViewportFlags_NoInputs, we do a search ourselves. Note that
...
void ImGui::UpdatePlatformWindows() {
    ...
    // io.MouseHoveredViewport. When setting Platform_GetWindowFocus, it is expected that the platform back-end can
...
static void DockContextClearNodes(ImGuiContext *ctx, ImGuiID root_id,
                                  bool clear_persistent_docking_refs);  // Use root_id==0 to clear all
...
static void DockNodePreviewDockCalc(ImGuiWindow *host_window, ImGuiDockNode *host_node, ImGuiWindow *payload_window,
...
static int DockNodeGetDepth(const ImGuiDockNode *node) {
    int depth = 0;
...
ImGuiID ImGui::DockContextGenNodeID(ImGuiContext *ctx) {
    ...
    // FIXME-OPT FIXME-DOCKING: This is suboptimal, even if the node count is small enough not to be a worry. We should
    // poke in ctx->Nodes to find a suitable ID faster.
    ImGuiID id = 0x0001;
...
static int IMGUI_CDECL DockNodeComparerDepthMostFirst(ImGuiDockNode *const *lhs, ImGuiDockNode *const *rhs) {
            ...
            if (payload_node->IsSplitNode()) {
                if (node->Windows.Size > 0) {
                    ...
                    IM_ASSERT(payload_node->OnlyNodeWithWindows !=
                              NULL);  // The docking should have been blocked by DockNodePreviewDockCalc() early on and
                                      // never submitted.
...
static void ImGui::DockNodeAddWindow(ImGuiDockNode *node, ImGuiWindow *window, bool add_to_tab_bar) {
    ...
    // If more than 2 windows appeared on the same frame, we'll create a new hosting DockNode from the point of the
    // second window submission. Then we need to hide the first window (after its been output) otherwise it would be
    // visible as a standalone window for one frame.
    ...
    if (node->HostWindow == NULL && node->IsFloatingNode()) {
        ...
        if (node->AuthorityForSize == ImGuiDataAuthority_Auto) node->AuthorityForSize = ImGuiDataAuthority_Window;
...
static void ImGui::DockNodeUpdate(ImGuiDockNode *node) {
    ...
    if (node->IsRootNode()) {
        ...
        // - Find if there's only a single visible window in the hierarchy (in which case we need to display a regular
        // title bar -> FIXME-DOCK: that last part is not done yet!)
        ...
        // When node has mixed windows, prioritize the class with the most constraint (DockingAllowUnclassed = false) as
    ...
    if (node->IsDockSpace()) {
        ...
        for (int window_n = 0; window_n < node->Windows.Size; window_n++) {
            // FIXME-DOCK: Setting DockIsActive here means that for single active window in a leaf node, DockIsActive
            // will be cleared until the next Begin() call.
            ImGuiWindow *window = node->Windows[window_n];
            window->DockIsActive = (node->Windows.Size > 1);
    ...
    // will only know the correct pos/size after processing the resizing splitters. So we are using the DrawList channel
...
static int IMGUI_CDECL TabItemComparerByDockOrder(const ImGuiTabItem *lhs, const ImGuiTabItem *rhs) {
...
static void ImGui::DockNodeUpdateTabBar(ImGuiDockNode *node, ImGuiWindow *host_window) {
    ...
    for (int window_n = 0; window_n < node->Windows.Size; window_n++) {
        ImGuiWindow *window = node->Windows[window_n];
        if (g.NavWindow && g.NavWindow->RootWindowDockStop == window) tab_bar->SelectedTabId = window->ID;
    ...
    for (int tab_n = tab_bar->Tabs.Size - 1; tab_n >= 0 && (tab_bar->Tabs[tab_n].Flags & ImGuiTabItemFlags_Unsorted);
         tab_n--) {
        // FIXME-DOCKING: Consider only clearing the flag after the tab has been alive for a few consecutive frames,
    ...
    if (tab_bar->Tabs.Size > tabs_unsorted_start) {
        ...
        if (tab_bar->Tabs.Size > tabs_unsorted_start + 1) {
            auto *data = tab_bar->Tabs.Data + tabs_unsorted_start;
            ImQsort(data, data + tab_bar->Tabs.Size - tabs_unsorted_start, TabItemComparerByDockOrder);
        }
    }
    ...
    for (int window_n = 0; window_n < node->Windows.Size; window_n++) {
        ...
        if (window->LastFrameActive + 1 >= g.FrameCount || !node_was_active) {
            ...
            if (g.NavWindow && g.NavWindow->RootWindowDockStop == window &&
                (window->DC.NavLayerActiveMask & (1 << 1)) == 0)
...
static void ImGui::DockNodeCalcTabBarLayout(const ImGuiDockNode *node, ImRect *out_title_rect, ImRect *out_tab_bar_rect,
    ...
    if (node->HasCloseButton) {
        r.Max.x -= g.FontSize;  // +1.0f; // In DockNodeUpdateTabBar() we currently display a disabled close button even
                                // if there is none.
    }
    if (node->HasWindowMenuButton && g.Style.WindowMenuButtonPosition == ImGuiDir_Left) {
        r.Min.x += g.FontSize;  // + g.Style.ItemInnerSpacing.x; // <-- Adding ItemInnerSpacing makes the title text
                                // moves slightly when in a docking tab bar. Instead we adjusted RenderArrowDockMenu()
...
bool ImGui::DockNodeCalcDropRectsAndTestMousePos(const ImRect &parent, ImGuiDir dir, ImRect &out_r, bool outer_docking,
    ...
    if (outer_docking) {
        ...
        off = ImVec2(ImFloor(parent.GetWidth() * 0.5f - GetFrameHeightWithSpacing() * 0.0f - hs_h),
                     ImFloor(parent.GetHeight() * 0.5f - GetFrameHeightWithSpacing() * 0.0f - hs_h));
    } else {
...
static void ImGui::DockNodePreviewDockCalc(ImGuiWindow *host_window, ImGuiDockNode *host_node,
    ...
    const bool src_is_visibly_splitted = root_payload->DockNodeAsHost && root_payload->DockNodeAsHost->IsSplitNode() &&
                                         (root_payload->DockNodeAsHost->OnlyNodeWithWindows == NULL);
    data->IsCenterAvailable = !is_outer_docking;
    if (src_is_visibly_splitted && (!host_node || !host_node->IsEmpty())) data->IsCenterAvailable = false;
    ...
    if (data->IsDropAllowed && can_preview_tabs && data->SplitDir == ImGuiDir_None && data->IsCenterAvailable) {
        ...
        for (int payload_n = 0; payload_n < payload_count; payload_n++) {
            ...
            ImGuiWindow *payload = root_payload->DockNodeAsHost
                                       ? root_payload->DockNodeAsHost->TabBar->Tabs[payload_n].Window
                                       : root_payload;
    ...
    if (child_0->IsVisible && child_1->IsVisible) {
        ...
        if ((child_0->GetMergedFlags() | child_1->GetMergedFlags()) & ImGuiDockNodeFlags_NoResize) {
            ...
            if (g.ActiveId == splitter_id) {
                ...
                GetForegroundDrawList((ImGuiViewportP*)GetMainViewport()); for (int n = 0; n < 2; n++) if (axis ==
                ImGuiAxis_X) draw_list->AddLine(ImVec2(resize_limits[n], node->ChildNodes[n]->Pos.y),
            ...
            if (SplitterBehavior(bb, GetID("##Splitter"), axis, &cur_size_0, &cur_size_1, min_size_0, min_size_1,
                ...
                if (touching_nodes[0].Size > 0 && touching_nodes[1].Size > 0) {
                    ...
                    for (int side_n = 0; side_n < 2; side_n++)
                        for (int touching_node_n = 0; touching_node_n < touching_nodes[side_n].Size;
                             touching_node_n++) {
                            ImGuiDockNode *touching_node = touching_nodes[side_n][touching_node_n];
                            // ImDrawList* draw_list = node->HostWindow ? GetForegroundDrawList(node->HostWindow) :
                            // GetForegroundDrawList((ImGuiViewportP*)GetMainViewport());
                            // draw_list->AddRect(touching_node->Pos, touching_node->Pos + touching_node->Size,
...
ImGuiDockNode *ImGui::DockNodeTreeFindNodeByPos(ImGuiDockNode *node, ImVec2 pos) {
    ...
    // There is an edge case when docking into a dockspace which only has inactive nodes (because none of the windows
    // are active) In this case we need to fallback into any leaf mode, possibly the central node.
    if (node->IsDockSpace() && node->IsRootNode()) {
        if (node->CentralNode && node->IsLeafNode())  // FIXME-20181220: We should not have to test for IsLeafNode()
                                                      // here but we have another bug to fix first.
...
// meaning we can't Begin/End in a single function. So if you want a menu bar you need to repeat this code manually
// ourselves. As with advanced other Docking API, we may change this function signature.
...
ImGuiID ImGui::DockBuilderAddNode(ImGuiID id, ImGuiDockNodeFlags flags) {
    ...
    if (flags & ImGuiDockNodeFlags_DockSpace) {
        ...
        if (!node) node = DockContextAddNode(ctx, id);
...
void ImGui::DockBuilderRemoveNodeChildNodes(ImGuiID root_id) {
    ...
    if (nodes_to_remove.Size > 1) {
        ImQsort(nodes_to_remove.Data, nodes_to_remove.Data + nodes_to_remove.Size - 1, DockNodeComparerDepthMostFirst);
    }
...
void ImGui::DockBuilderRemoveNodeDockedWindows(ImGuiID root_id, bool clear_persistent_docking_references) {
    ...
    for (int n = 0; n < g.Windows.Size; n++) {
        ...
        if (want_removal) {
            ...
            if (!clear_persistent_docking_references) IM_ASSERT(window->DockId == backup_dock_id);
        }
    }
}
...
static void ImGui::DockSettingsRenameNodeReferences(ImGuiID old_node_id, ImGuiID new_node_id) {
    ...
    for (int settings_n = 0; settings_n < g.SettingsWindows.Size;
         settings_n++)  // FIXME-OPT: We could remove this loop by storing the index in the map
    {
        ImGuiWindowSettings *window_settings = &g.SettingsWindows[settings_n];
        if (window_settings->DockId == old_node_id) window_settings->DockId = new_node_id;
    }
}
...
static ImGuiDockNodeSettings *ImGui::DockSettingsFindNodeSettings(ImGuiContext *ctx, ImGuiID id) {
    ...
    for (int n = 0; n < dc->SettingsNodes.Size; n++)
        if (dc->SettingsNodes[n].ID == id) return &dc->SettingsNodes[n];
    return NULL;
}
...
static void ImGui::DockSettingsHandler_WriteAll(ImGuiContext *ctx, ImGuiSettingsHandler *handler,
    ...
    for (int node_n = 0; node_n < dc->SettingsNodes.Size; node_n++) {
        ...
        if (ImGuiDockNode *node = DockContextFindNodeByID(ctx, node_settings->ID)) {
            ...
            for (int window_n = 0; window_n < ctx->SettingsWindows.Size;
                 window_n++)  // Iterate settings so we can give info about windows that didn't exist during the
                              // session.
...
void ImGui::ShowViewportThumbnails() {
    ...
    for (int n = 0; n < g.Viewports.Size; n++) bb_full.Add(g.Viewports[n]->GetRect());
...
void ImGui::ShowMetricsWindow(bool *p_open) {
    ...
    struct Funcs {
        ...
        static void NodeDrawList(ImGuiWindow *window, ImGuiViewportP *viewport, ImDrawList *draw_list,
                                 const char *label) {
            bool node_open =
        ...
        static void NodeViewport(ImGuiViewportP *viewport) {
            ...
            if (ImGui::TreeNode((void *) (intptr_t) viewport->ID,
                ...
                ImGui::BulletText("Pos: (%.0f,%.0f), Size: (%.0f,%.0f), Monitor: %d, DpiScale: %.0f%%", viewport->Pos.x,
                                  viewport->Pos.y, viewport->Size.x, viewport->Size.y, viewport->PlatformMonitor,
        ...
        static void NodeDockNode(ImGuiDockNode *node, const char *label) {
            ...
            if (open) {
                ...
                ImGui::BulletText("Pos (%.0f,%.0f), Size (%.0f, %.0f) Ref (%.0f, %.0f)", node->Pos.x, node->Pos.y,
                                  node->Size.x, node->Size.y, node->SizeRef.x, node->SizeRef.y);
            ...
            if (ImGui::TreeNode(tab_bar, "%s", buf)) {
                for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
                    ...
                    ImGui::Text("%02d%c Tab 0x%08X '%s'", tab_n, (tab->ID == tab_bar->SelectedTabId) ? '*' : ' ',
                                tab->ID, tab->Window ? tab->Window->Name : "N/A");
                    ImGui::PopID();
                }
                ImGui::TreePop();
            }
    ...
    if (ImGui::TreeNode("Viewport", "Viewports (%d)", g.Viewports.Size)) {
        ...
        if (g.PlatformIO.Monitors.Size > 0 &&
            ImGui::TreeNode("Monitors", "Monitors (%d)", g.PlatformIO.Monitors.Size)) {
            ImGui::TextWrapped(
                "(When viewports are enabled, imgui needs uses monitor data to position popup/tooltips so they don't "
                "straddle monitors.)");
    ...
    if (ImGui::TreeNode("Docking")) {
        ...
        if (ImGui::TreeNode("Dock nodes")) {
            if (ImGui::SmallButton("Clear settings")) {
        ...
        if (ImGui::TreeNode("Settings")) {
            if (ImGui::SmallButton("Refresh")) SaveIniSettingsToMemory();
            ImGui::SameLine();
            if (ImGui::SmallButton("Save to disk")) SaveIniSettingsToDisk(g.IO.IniFilename);
            ImGui::Separator();
            ImGui::Text("Docked Windows:");
            for (int n = 0; n < g.SettingsWindows.Size; n++)
                if (g.SettingsWindows[n].DockId != 0)
                    ImGui::BulletText("Window '%s' -> DockId %08X", g.SettingsWindows[n].Name,
                                      g.SettingsWindows[n].DockId);
            ImGui::Separator();
    ...
    if (ImGui::TreeNode("Tools")) {
        ...
        show_windows_rects |=
            ImGui::Combo("##show_windows_rect_type", &show_windows_rect_type, wrt_rects_names, WRT_Count);
    ...
    if (show_docking_nodes && g.IO.KeyCtrl) {
        for (int n = 0; n < g.DockContext->Nodes.Data.Size; n++)
            if (ImGuiDockNode *node = (ImGuiDockNode *) g.DockContext->Nodes.Data[n].val_p) {
                ...
                char buf[64] = "";
                char *p = buf;
                ImDrawList *overlay_draw_list = node->HostWindow
                ...
                overlay_draw_list->AddRect(node->Pos + ImVec2(3, 3) * (float) depth,
                                           node->Pos + node->Size - ImVec2(3, 3) * (float) depth,
                                           IM_COL32(200, 100, 100, 255));
