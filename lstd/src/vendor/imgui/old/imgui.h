...
IMGUI_API void SetCursorScreenPos(
    const ImVec2 &pos);  // cursor position in absolute screen coordinates (0..io.DisplaySize) or natural OS coordinates
...
IMGUI_API void SetNextWindowClass(
    const ImGuiWindowClass *window_class);  // set next window class (rare/advanced uses: provide hints to the platform
...
IMGUI_API void RenderPlatformWindowsDefault(
    ...
    void *renderer_arg = NULL);  // call in main loop. will call RenderWindow/SwapBuffers platform functions for each
                                 // secondary viewport which doesn't have the ImGuiViewportFlags_Minimized flag set. May
                                 // be reimplemented by user for custom rendering needs.
IMGUI_API void DestroyPlatformWindows();  // call DestroyWindow platform functions for all viewports. call from back-end
...
enum ImGuiBackendFlags_ {
    ...
    ImGuiBackendFlags_HasMouseHoveredViewport =
        1 << 11,  // Back-end Platform supports setting io.MouseHoveredViewport to the viewport directly under the mouse
                  ...
                  // correctly with most high-level engines! Don't set this without studying how the examples/ back-end
                  // handle it!
...
struct ImGuiIO {
    ...
    ImVec2 DisplaySize;   // <unset>          // Main display size, in pixels. This is for the default viewport. Use
                          // BeginViewport() for other viewports.
    ...
    bool ConfigWindowsMoveFromTitleBarOnly;  // = false       // [BETA] Set to true to only allow moving windows when
                                             ...
                                             // affected.
    float ConfigWindowsMemoryCompactTimer;   // = 60.0f          // [BETA] Compact window memory usage when unused. Set
                                             // to -1.0f to disable.
...
struct ImGuiWindowClass {
    ...
    bool DockingAlwaysTabBar;    // Set to true to enforce windows of this class always having their own tab (equivalent
                                 // of setting the global io.ConfigDockingAlwaysTabBar)
...
// - this is used for back-ends aiming to support the seamless creation of multiple viewport (= multiple Platform/OS
// windows)
...
struct ImGuiPlatformIO {
    ...
    void (*Platform_ShowWindow)(ImGuiViewport *vp);  // Newly created windows are initially hidden so
                                                     // SetWindowPos/Size/Title can be called on them first
    ...
    void (*Platform_UpdateWindow)(
        ImGuiViewport *vp);  // (Optional) Called in UpdatePlatforms(). Optional hook to allow the platform back-end
                             // from doing general book-keeping every frame.
    ...
    void (*Platform_SwapBuffers)(ImGuiViewport *vp,
                                 void *render_arg);          // (Optional) Call Present/SwapBuffers (platform side)
    float (*Platform_GetWindowDpiScale)(ImGuiViewport *vp);  // (Optional) [BETA] (FIXME-DPI) DPI handling: Return DPI
                                                             // scale for this viewport. 1.0f = 96 DPI.
    void (*Platform_OnChangedViewport)(
        ImGuiViewport
            *vp);  // (Optional) [BETA] (FIXME-DPI) DPI handling: Called during Begin() every time the viewport we are
                   // outputting into changes, so back-end has a chance to swap fonts to adjust style.
    void (*Platform_SetImeInputPos)(ImGuiViewport *vp,
                                    ImVec2 pos);  // (Optional) Set IME (Input Method Editor, e.g. for Asian languages)
                                                  // input position, so text preview appears over the imgui input box.
    int (*Platform_CreateVkSurface)(ImGuiViewport *vp, ImU64 vk_inst, const void *vk_allocators,
                                    ImU64 *out_vk_surface);  // (Optional) For Renderer to call into Platform code
    ...
    void (*Renderer_RenderWindow)(ImGuiViewport *vp,
                                  void *render_arg);  // (Optional) Clear targets, Render viewport->DrawData
    void (*Renderer_SwapBuffers)(ImGuiViewport *vp,
                                 void *render_arg);  // (Optional) Call Present/SwapBuffers (renderer side)

    // (Optional) List of monitors (updated by: app/back-end, used by: imgui to clamp popups/tooltips within same
...
enum ImGuiViewportFlags_ {
    ...
    ImGuiViewportFlags_NoRendererClear = 1 << 5,  // Platform Window: Renderer doesn't need to clear the framebuffer
    ...
    ImGuiViewportFlags_NoAutoMerge =
        1 << 8,  // Platform Window: Avoid merging this widow into another host window. This can only be set via
    ...
    ImGuiViewportFlags_CanHostOtherWindows =
        1
        << 9  // Main viewport: can host multiple imgui windows (secondary viewports are associated to a single window).
};

// The viewports created and managed by imgui. The role of the platform back-end is to create the platform/OS windows
...
struct ImGuiViewport {
    ...
    void *
        RendererUserData;  // void* to hold custom data structure for the renderer (e.g. swap chain, frame-buffers etc.)
    ...
    void *PlatformHandleRaw;    // void* to hold low-level, platform-native window handle (e.g. the HWND) when using an
